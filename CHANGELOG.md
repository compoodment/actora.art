...
- Added bounded public-page reading so Aurora can inspect visitor-provided websites and public GitHub repositories, including relevant linked repository files.
- actoraOS is now `0.2.509`.
...
- Each moon uses its real Cassini-derived shape and stays oriented toward Saturn. They intentionally begin with neutral, texture-free surfaces because no trustworthy compatible global maps were found.
- Existing 40-body and older 38-body Solar saves keep their exact saved rosters; a reset or new session uses the expanded system.
- actoraOS is now `0.2.460`.
...
- A single clear completion now appears as dim inline ghost text. When several commands or page names match, a small unboxed rail keeps arrow-key selection, descriptions, Escape dismissal, and Tab acceptance without adding permanent status chrome.
- `page` is now the one navigation command: use it alone for a selectable six-page picker or type `page <name>` to go directly. The former `ls` and `cd` commands are gone, and `pages` is not an alias.
- Account and Messages remain visible in the picker with their sign-in requirement and still verify the current session before opening. The terminal adds no borrowed product logo, user/session banner, or permanent ready label.
- actoraOS is now `0.2.446`.
...
- Kept the footer `social` dialog as the fast notification, recent-preview, System-action, and quick-reply layer, with an `open all messages` link into the full inbox. Profile actions, the footer, and Messages now share the same Social behavior, including accurate in-view unread acknowledgement and immediate clearing when a session ends.
- Reduced duplicate live Social connections across tabs while keeping account changes isolated, and kept private search bounded to visible retained conversations. Messages adds no report placeholder, public friend/follower counts, likes, views, rankings, or other popularity metrics. actoraOS is now `0.2.404`.
...
- Added dedicated Alpha Centauri surface textures, multi-star lighting and trails, full loading coverage when changing systems, and saves that reopen the system they were created in.

## 0.2.387 - 2026-07-12

- Added a system selector to Space's desktop and mobile controls, with Solar System remaining the default and the interface ready to host future implemented systems.
...
- Made the Space Sun more realistic with a warmer white-light surface, natural edge darkening, a finer red chromosphere, and fewer prominence loops clustered into active regions instead of surrounding the whole star.
...
- Fixed the Space left control panel and game menu layout after the Create and Missions cleanup.
