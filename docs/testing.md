...
Use this after auth or API hardening changes.

1. Start public passkey registration with a reserved staff/system-looking username.
   - Expected: registration is rejected with a small public error.
2. Start passkey registration and login with normal inputs.
   - Expected: returned WebAuthn options set `userVerification` to `required`.
3. Request a dynamic JSON API such as `/api/auth/me`.
   - Expected: the response includes `Cache-Control: no-store`, `Pragma: no-cache`, and `Expires: 0`.
4. Ask Aurora to share or analyze implementation source code.
   - Expected: Aurora refuses briefly and does not quote, summarize, or dump source code, while still allowing docs/changelog/release-level questions.
...
2. Send a harmless message.
   ...
   - Expected: replies with explicit queer/pride trigger words render the whole assistant message in rainbow text, while matching trigger words such as `queer` and `lesbians` stay visible and loop the extra rainbow letter-split animation.
   - Expected: if someone comments on seeing the rainbow/light-show effect, Aurora acknowledges it as real site behavior instead of denying it.
...
5. Archive a chat, open the archived section, and select the archived chat.
   ...
   - Expected: the chat help hint says archived chats auto-delete after 7 days and does not mention internal account categories.
...
5. Open Settings and switch through Controls, Graphics, Audio, Accessibility, and Gameplay.
   ...
   - Expected: head bob is enabled by default, follows actual meter-scale ground distance rather than elapsed time, has a noticeable but light walking rhythm with slight pitch/side roll, speeds up appropriately while sprinting, pauses/resets while the menu is open or movement stops, and can be disabled without changing movement, jump, camera position, or collision behavior.
6. Open Help.
   - Expected: help uses normal game control labels: `move: W A S D`, `sprint: Shift`, `jump: Space`, `look: Mouse`, and `menu / back: Escape`. On short viewports, the projected options panel scrolls instead of clipping controls.
7. Use `restart`, resume, then exit to lab.
   - Expected: restart resets only player position/look and does not reset settings; resume returns to the room; focus returns to the menu when it opens; menu controls work while pointer lock is released; exit navigates to `/lab`.
8. Test pointer lock denial/unavailability if practical.
   - Expected: the menu stays visible and shows a plain mouse-look error instead of leaving a blank non-movable room.
