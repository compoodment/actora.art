...
- The footer `social` dialog stays available as the compact notification, recent-preview, `System` action, friends launcher, and quick-reply layer. Friends appear as square avatar/name buttons, `open messages/` moves into the full inbox, and an active DM avatar opens the other person's profile.
