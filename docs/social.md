...
Profile Music can be managed beside items in Music or through `+ > music` while editing your own profile. The profile option opens a searchable picker for songs, albums, and playlists in your Library; private playlists explain that they must be made public before they can appear on a profile. Each plus/minus saves immediately, up to eight mixed items, and new choices append. The showcase itself remains read-only, so profile Save, Cancel, Undo, and Redo do not change the Music selection. Deleted or unavailable Music and playlists made private disappear from the showcase.
