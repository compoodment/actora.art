...
It is a separate game-mode surface rather than a generic site experiment panel. New lives run Actora's tagged `v0.58.0` Python command/save runtime in a dedicated browser Worker, while existing local TypeScript lives and unfinished character drafts stay on their compatible legacy engine. The current public version uses a dark, desktop-first life-ledger presentation with a persistent character rail, flat ruled workspaces, required Character Creation, the full prototype questionnaire, the current prototype geography set of 12 countries and 43 cities, and a Year 1 / Month 1 newborn local-life start.

The prototype opens on a save-aware title screen with a direct return to Lab: begin a life when no healthy local save exists, or review and continue the current character. Starting a New Life from the title, active game, or terminal continuation state uses one protected confirmation. Character Creation keeps its active six- or seven-step path truthful and returns from confirmation without losing the chosen mode or details.

It includes generated family setup, later sibling births, meeting NPCs, lifecycle identity choices, local event history, action records, month-by-month advancement, pending choices, in-game relationship links from meeting choices, Hang out actions, and Life/Actions/Profile/Relationships/History sections. Life puts the live story first and keeps its overview plus compact Advance/Skip controls alongside it. Actions groups Social and Personal choices in one compact hierarchy beside the queue and time budget. Relationships is the sole family/social index and uses one searchable people-and-detail browser, while Profile uses label-only navigation for six factual sections and shows each fact once in its inspector; Mood + Needs and Skills / Talents remain honest unavailable placeholders. The live feed remains immediate story context rather than replacing the complete History, whose grouped chronology avoids exposing negative internal years. First-pass death continuation moves through a finished-life summary, available linked lives, candidate detail, and explicit confirmation.
